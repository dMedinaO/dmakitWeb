...
$query = "select count(CAST(job.createdJob  as DATE)) as cantidad, CAST(job.createdJob  as DATE) as fecha from job where job.user = $idUser AND job.tipo_job in ('CORRELATION', 'MUTUAL-INFORMATION', 'PCA', 'PCA-INCREMENTAL', 'KERNEL-PCA', 'SPATIAL DEFORMATION') group by CAST(job.createdJob   as DATE)";
