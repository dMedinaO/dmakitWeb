...
$query = "select count(CAST(job.createdJob  as DATE)) as cantidad, CAST(job.createdJob  as DATE) as fecha from job where job.user = $idUser AND job.tipo_job in ('VIEW CONTINUOS DATA', 'DISTRIBUTION', 'FREQUENCE', 'PARALLEL-COORDINATES', 'SPLOM', 'DISPERSION', 'STATISTIC SUMMARY') group by CAST(job.createdJob   as DATE)";
