...
$idUSer = $_SESSION['idUser'];
  ...
  if ($requestData == "BIEN"){#movemos el archivo de tmp al path del usuario y ejecutamos el proceso solo si la opcion de algorithm es todos...
    ...
    $command = "python /var/www/html/smartTraining/model/launcherStatisticalProcessOption.py $idUSer $idJob $pathMove$nameDocument $pathRespone 2";
    $response['command2'] = $command;

    exec($command);
    $responseFile = "../../../dataStorage/$idUSer/$idJob/responseDispersion$idJob.json";
    $responseData = file_exists("/var/www/html/smartTraining/dataStorage/$idUSer/$idJob/responseDispersion$idJob.json");
